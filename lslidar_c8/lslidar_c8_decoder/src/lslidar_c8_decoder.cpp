...
LslidarC8Decoder::LslidarC8Decoder(
    ...
    sweep_data(new lslidar_c8_msgs::LslidarC8Sweep()),
    multi_scan(new lslidar_c8_msgs::LslidarC8Layer())
    {
    return;
}

bool LslidarC8Decoder::loadParameters() {
...
bool LslidarC8Decoder::createRosIO() {
    packet_sub = nh.subscribe<lslidar_c8_msgs::LslidarC8Packet>(
                "lslidar_packet", 100, &LslidarC8Decoder::packetCallback, this);
    layer_sub = nh.subscribe(
                "layer_num", 100, &LslidarC8Decoder::layerCallback, this);
    sweep_pub = nh.advertise<lslidar_c8_msgs::LslidarC8Sweep>(
                "lslidar_sweep", 10);
    point_cloud_pub = nh.advertise<sensor_msgs::PointCloud2>(
                "lslidar_point_cloud", 10);
    scan_pub = nh.advertise<sensor_msgs::LaserScan>(
                "scan", 100);
    channel_scan_pub = nh.advertise<lslidar_c8_msgs::LslidarC8Layer>(
                "scan_channel", 100);
    return true;
}

bool LslidarC8Decoder::initialize() {
    if (!loadParameters()) {
...
bool LslidarC8Decoder::checkPacketValidity(const RawPacket* packet) {
...
void LslidarC8Decoder::publishPointCloud() {
    ...
    for (size_t i = 0; i < 16; ++i) {
        const lslidar_c8_msgs::LslidarC8Scan& scan = sweep_data->scans[i];
...
void LslidarC8Decoder::publishChannelScan()
{
    multi_scan = lslidar_c8_msgs::LslidarC8LayerPtr(
                    new lslidar_c8_msgs::LslidarC8Layer());
    // lslidar_c8_msgs::LslidarC8Layer multi_scan(new lslidar_c8_msgs::LslidarC8Layer);
...
void LslidarC8Decoder::publishScan()
{
...
point_struct LslidarC8Decoder::getMeans(std::vector<point_struct> clusters)
{
    point_struct tmp;
...
void LslidarC8Decoder::decodePacket(const RawPacket* packet) {
...
void LslidarC8Decoder::layerCallback(const std_msgs::Int8Ptr& msg){
    int num = msg->data;
    if (num < 0)
    {
        num = 0;
...
void LslidarC8Decoder::packetCallback(
        const lslidar_c8_msgs::LslidarC8PacketConstPtr& msg) {
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
        for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
            ...
            sweep_data->scans[remapped_scan_idx].points.push_back(
                        lslidar_c8_msgs::LslidarC8Point());

            lslidar_c8_msgs::LslidarC8Point& new_point =		// new_point 为push_back最后一个的引用
    ...
    if (end_fir_idx != FIRINGS_PER_PACKET) {
        ...
        sweep_data = lslidar_c8_msgs::LslidarC8SweepPtr(
                    new lslidar_c8_msgs::LslidarC8Sweep());
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
                ...
                sweep_data->scans[remapped_scan_idx].points.push_back(
                            lslidar_c8_msgs::LslidarC8Point());
                lslidar_c8_msgs::LslidarC8Point& new_point =
                        sweep_data->scans[remapped_scan_idx].points[
                        sweep_data->scans[remapped_scan_idx].points.size()-1];
...
} // end namespace lslidar_c8_decoder
}
}
