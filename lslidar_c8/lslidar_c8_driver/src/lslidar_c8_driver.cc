...
bool LslidarC8Driver::loadParameters() {
...
bool LslidarC8Driver::createRosIO() {

  // ROS diagnostics
  diagnostics.setHardwareID("Lslidar_C8");
  // c8 publishs 20*16 thousands points per second.
  ...
  ROS_INFO("expected frequency: %.3f (Hz)", diag_freq);
    ...
    // Output
    packet_pub = nh.advertise<lslidar_c8_msgs::LslidarC8Packet>(
                "lslidar_packet", 100);
    return true;
}

bool LslidarC8Driver::openUDPPort() {
...
bool LslidarC8Driver::initialize() {
    ...
    ROS_INFO("Initialised lslidar c8 without error");
    return true;
}

int LslidarC8Driver::getPacket(
        lslidar_c8_msgs::LslidarC8PacketPtr& packet) {
...
bool LslidarC8Driver::polling()
{
    // Allocate a new shared pointer for zero-copy sharing with other nodelets.
    lslidar_c8_msgs::LslidarC8PacketPtr packet(
                new lslidar_c8_msgs::LslidarC8Packet());
...
void LslidarC8Driver::initTimeStamp(void)
{
    int i;
...
void LslidarC8Driver::getFPGA_GPSTimeStamp(lslidar_c8_msgs::LslidarC8PacketPtr &packet)
{
    ...
    if(head2[0] == 0xA5 && head2[1] == 0xFF)
    {
        ...
        {   
            ...
            else if (cnt_gps_ts == 1)
            {
                ...
                ROS_INFO("current stable timestamp is %lu", GPSStableTS);
            }
            else
            {
                cnt_gps_ts ++;
                ROS_INFO("already store gps ts with cnt %d", cnt_gps_ts);
            }
    ...
    else if(head2[0] == 0xFF && head2[1] == 0xEE)
    {
        ...
        {
            GPS_ts = GPSStableTS;
            ROS_FATAL("This is step time, using new GPS ts %lu, last is %lu, current is %lu", GPS_ts, last_FPGA_ts, packet_timestamp);
        }
        ...
        ROS_INFO("ROS TS: %f, GPS: %lu; FPGA: us:%lu",
